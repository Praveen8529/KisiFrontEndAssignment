...
import UploaderDeatils from "./Detail";
import "./slider.css";

const Slider = ({ slider, handlerNavigation }) => {
  const [active, setActive] = useState(0);
  const [slides] = useState([{ eachSlide: slider.modalSrc }]);
  const [autoplay, setAutoplay] = useState(0);
  const max = slides.length;
  const { imageIndex, modalSrc } = slider;
  ...
  const renderSlides = () => (
    <div
      ...
      key={imageIndex}
      style={{ backgroundImage: modalSrc }}
    ></div>
  );

  const renderDots = () =>
    Array.from("12345").map((
      silde,
      index // check index
    ) => (
  ...
  const renderArrows = () => (
    <>
      <button
        ...
        onClick={() => handlerNavigation(null, imageIndex - 1)}
      >
